...
export const columns: ColumnDef<MeetingGetMany[number]>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge
          variant="outline"
          className={cn(
            'text-muted-foreground capitalize [&_svg]:size-4',
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Badge
        variant="outline"
        className="text-muted-foreground flex items-center gap-x-2 capitalize [&_svg]:size-4"
      >
        <ClockArrowUpIcon className="text-blue-700" />
        {row.original.duration
          ? format(row.original.duration, 'MMM d')
          : 'No duration'}
      </Badge>
    ),
  },
